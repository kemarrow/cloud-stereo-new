...
while(1):
    ...
    fig3, (ax3, ax4, ax5) = plt.subplots(3,1, sharex =  True, sharey =True)
    ...
    spd = ax3.imshow(speed, vmin = 0, vmax = 200)
    ...
    fastpix = speed 
    fastpix[fastpix < 20] =  None
    ax4.imshow(masked_left)
    spd = ax4.imshow(fastpix, vmin = 0, vmax = 200)
    ax4.set_xlabel('more than 20m/s')
    dividerh = make_axes_locatable(ax4)
    cax3 = dividerh.append_axes('right', size='5%', pad=0.05)
    fig3.colorbar(spd, cax=cax3, orientation='vertical')

    slowpix = speed
    slowpix[slowpix > 5]  = None
    ax5.imshow(masked_left)
    ax5.set_xlabel('less than 5m/s')
    spd = ax5.imshow(slowpix, vmin = 0, vmax = 5)
    dividerh = make_axes_locatable(ax5)
    cax3 = dividerh.append_axes('right', size='5%', pad=0.05)
    fig3.colorbar(spd, cax=cax3, orientation='vertical')


    ax3.set_title(date + str(count)) 
    #ax3.set_xlabel('speed')

    counting = str(count)
    
    file_name = 'gif_speeds/speeds' + counting.zfill(5) + '.png'
    fig3.savefig(file_name, format='png', dpi = 500)
