...
def opencv2_setup():
    #folder = '/net/seldon/disk1/Users/erg10/timelapse/videos/'
    #dtime = '2021-03-11_16'
    #dtime = '2021-03-13_14'
    ...
    #stereo_cal = get_stereo_cal(prefixleft, prefixright)
    w, h = 640, 480
    
    mapxl, mapyl = cv2.initUndistortRectifyMap(
        rpi2_camera_matrix, rpi2_camera_dist,
        stereo_cal['R1'], stereo_cal['P1'], (w, h), 5)
    mapxr, mapyr = cv2.initUndistortRectifyMap(
        rpi2_camera_matrix, rpi2_camera_dist,
        stereo_cal['R2'], stereo_cal['P2'], (w, h), 5)
    
    vidcap = cv2.VideoCapture(f'{folder}{dtime}/{prefixright}_{dtime}_{hour:0>2}A.mp4')
    vidcap2 = cv2.VideoCapture(f'{folder}{dtime}/{prefixleft}_{dtime}_{hour:0>2}A.mp4')
